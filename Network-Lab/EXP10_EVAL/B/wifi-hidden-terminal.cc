...
void experiment(bool enableCtsRts, std::string wifiManager)
{
  ...
  //   }
  //   nodes.Get(0)->GetObject<MobilityModel>()->SetPosition(Vector(100,0,0));
  ...
  nodes.Get(1)->AggregateObject(CreateObject<ConstantPositionMobilityModel>());
  ...
  s1->SetPosition(Vector(-100, 0, 0));
  s2->SetPosition(Vector(0, 0, 0));
  s3->SetPosition(Vector(100, 0, 0));

  // 3. Create propagation loss matrix
  Ptr<FriisPropagationLossModel> lossModel = CreateObject<FriisPropagationLossModel>();
  lossModel->SetMinLoss(50); // set default loss to 200 dB (no link)
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin(); i != stats.end(); ++i)
  {
    ...
    if (i->first > 2)
    {
      ...
      std::cout << "  Packet Loss Ratio: " << ((float)(i->second.txPackets) - (i->second.rxPackets)) / (float)(i->second.txPackets) << "\n";
    }
  }
