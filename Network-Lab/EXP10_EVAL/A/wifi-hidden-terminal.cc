...
void experiment(bool enableCtsRts, std::string wifiManager)
{
  ...
  s1->SetPosition(Vector(-100, 50, 0));
  s2->SetPosition(Vector(0, 50, 0));
  s3->SetPosition(Vector(100, 50, 0));
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin(); i != stats.end(); ++i)
  {
    ...
    if (i->first > 2)
    {
      ...
      std::cout << "  Packet Loss Ratio: " << ((i->second.txPackets - i->second.rxPackets) * 1.0) / i->second.txPackets << " \n";
    }
  }
